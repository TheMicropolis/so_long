...
{
	game->player.dx = x - game->player.x;
	game->player.dy = y - game->player.y;
	if (game->map.array == NULL || y < 0 || x < 0
		|| y >= game->map.map_height || x >= game->map.map_width)
		exit_game(game, 1);
	...
	if ((game->player.y == game->map.exit_y)
		&& (game->player.x == game->map.exit_x))
		game->map.array[y - game->player.dy][x - game->player.dx] = 'E';
	else
		game->map.array[y - game->player.dy][x - game->player.dx] = '0';
	game->player.moves++;
